void draw_oneX_vs_Y(double offset)
{
	...
	char filename[100]; sprintf(filename,"Iteration_PostionLC1_inclusive_X_-%.2f.txt",Xoffset);
	...
	gMeanZZ1VsY->GetXaxis()->SetTitleSize(0.05); gMeanZZ1VsY->GetXaxis()->SetLabelSize(0.05); gMeanZZ1VsY->GetYaxis()->SetTitleSize(0.05); gMeanZZ1VsY->GetYaxis()->SetLabelSize(0.05); gMeanZZ1VsY->GetXaxis()->LabelsOption("u");
...
//  fout<<"\t"<<-1.0*f1->GetParameter(1)/(2.0*f1->GetParameter(2))<<endl;
  ...
  //fout<<"\t"<<-1.0*f2->GetParameter(1)/(2.0*f2->GetParameter(2))<<endl;
	...
	char pngname[40]; sprintf(pngname,"./Vs_Y/X_%.2f.pdf",offset);
