...
void main(){
  ...
  int NHitsREF2X[NN],NHitsREF2Y[NN],NHitsREF3X[NN],NHitsREF3Y[NN],NHitsUVA3X[NN],NHitsUVA3Y[NN],NHitsREF1X[NN],NHitsREF1Y[NN];
  //positions on detectors
  ...
  for(int i=0;i<NN;i++){
    ...
    Bool_t cutNHitsREF2=kFALSE; if(NHitsREF2X[i]>0 && NHitsREF2Y[i]>0) {cutNHitsREF2=kTRUE;}//cout<<"REF2 true"<<endl;}
    Bool_t cutNHitsREF3=kFALSE; if(NHitsREF3X[i]>0 && NHitsREF3Y[i]>0) {cutNHitsREF3=kTRUE;}//cout<<"REF3 true"<<endl;}
    Bool_t cutNHitsUVA3=kFALSE; if(NHitsUVA3X[i]>0 && NHitsUVA3Y[i]>0) {cutNHitsUVA3=kTRUE;}//cout<<"UVA3 true"<<endl;}
    Bool_t cutNHitsREF1=kFALSE; if(NHitsREF1X[i]>0 && NHitsREF1Y[i]>0) {cutNHitsREF1=kTRUE;}//cout<<"REF1 true"<<endl;}
    Bool_t cutNHitsEta1=kFALSE; if(NHitsEta1[i]>0) cutNHitsEta1=kTRUE;
    Bool_t cutNHitsEta2=kFALSE; if(NHitsEta2[i]>0) cutNHitsEta2=kTRUE;
    Bool_t cutNHitsEta3=kFALSE; if(NHitsEta3[i]>0) cutNHitsEta3=kTRUE;
    ...
    Bool_t cutpREF2X=kFALSE; if(pREF2X[i]>=7.5 && pREF2X[i]<=7.9) cutpREF2X=kTRUE;
    Bool_t cutpREF1X=kFALSE; if(pREF1X[i]>=-44 && pREF1X[i]<=-42) cutpREF1X=kTRUE;
    Bool_t cutpREF3X=kFALSE; if(pREF3X[i]>=54 && pREF3X[i]<=56) cutpREF3X=kTRUE;
    Bool_t cutpUVA3X=kFALSE; if(pUVA3X[i]>=50 && pUVA3X[i]<=52) cutpUVA3X=kTRUE;
    Bool_t cutpREF2Y=kFALSE; if(pREF2Y[i]>=-10 && pREF2Y[i]<=-5) cutpREF2Y=kTRUE;
    Bool_t cutpREF1Y=kFALSE; if(pREF1Y[i]>=-4 && pREF1Y[i]<=8) cutpREF1Y=kTRUE;
    ...
    Bool_t trigger=kFALSE; if(cutNHitsREF2==kTRUE && cutNHitsREF3 && cutNHitsUVA3==kTRUE && cutNHitsREF1 ) trigger=kTRUE;
    if(trigger ){
        totalEvents++;
        fout<<pREF2X[i]<<"\t"<<pREF2Y[i]<<"\t"<<pREF3X[i]<<"\t"<<pREF3Y[i]<<"\t"<<pUVA3X[i]<<"\t"<<pUVA3Y[i]<<"\t"
            <<pREF1X[i]<<"\t"<<pREF1Y[i]<<endl;//"\t"<<pEta5[i]-shiEta5<<endl;
        hpREF2X->Fill(pREF2X[i]); hpREF2Y->Fill(pREF2Y[i]); hpREF3X->Fill(pREF3X[i]); hpREF3Y->Fill(pREF3Y[i]);
